...
1. NOTION (MCP) — SOPs, policies, leasing procedures, checklists. Search first for any policy question. Always cite which Notion page.
2. APTLY (MCP) — Leads pipeline, workflow boards (Move-Ins, Move-Outs, HOA Violations, Tenant Renewals, Renter Leads), contacts.
3. SLACK (MCP) — Team messages, announcements, decisions.
4. RENTVINE (tools) — Live properties, leases, tenants, owners, work orders. Source of truth for all live property data.
...
// ── Claude API proxy — key stays on server, never exposed to browser ──────────
app.post('/api/chat', async (req, res) => {
  ...
  try {
    const { messages, tools, mcp_servers } = req.body;
...
app.get('/health', (req, res) => res.json({ status: 'ok', hasKey: !!ANTHROPIC_API_KEY }));
...
function Assistant() {
  ...
  const rvTools = cfg.rvKey ? [
    ...
    {name:"get_work_orders",description:"Fetch maintenance work orders",input_schema:{type:"object",properties:{property_id:{type:"string"},status:{type:"string"}}}},
    {name:"get_owner_info",description:"Look up owner info",input_schema:{type:"object",properties:{query:{type:"string"}},required:["query"]}},
  ] : [];
  ...
  // All Claude calls go through /api/chat on the server — API key never in browser
  const callClaude = async (msgs, withTools) => {
    ...
    if (cfg.gdUrl) mcpServers.push({type:"url",url:cfg.gdUrl,name:"gdrive"});
    ...
    const res = await fetch('/api/chat',{
      ...
      body:JSON.stringify({
        ...
        ...(withTools&&allTools.length>0&&{tools:allTools}),
      }),
    });
              ...
              <div style={{minHeight:400,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:24}}>
                ...
                <div style={{display:"flex",flexWrap:"wrap",gap:6,justifyContent:"center",maxWidth:440}}>
                  {Object.entries(SOURCES).map(([key,s]) => {
                    ...
                    return React.createElement('div',{key,style:{padding:"3px 9px",borderRadius:20,background:on?s.bg:"#f5f5f5",border:\`1px solid \${on?s.border:"#e5e5e5"}\`,fontSize:12,color:on?"#333":"#aaa"}},s.label);
                  })}
                </div>
