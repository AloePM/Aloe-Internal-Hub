...
app.post('/api/chat', async function(req, res) {
  ...
  try {
    ...
    const isAvailabilityQ = lowerMsg.match(/availab|for rent|vacant|what unit|what prop|what home|what listing|what house|listing|under \$|under \d|homes.*\$|homes.*rent|rent.*homes|bedroom|2 bed|3 bed|4 bed|how many.*home|how many.*unit/) && !lowerMsg.match(/[0-9]{5,6}/);
    ...
    // Extract price filter if present (e.g. "under $1800", "under 1800", "less than $2000")
    const priceMatch = lowerMsg.match(/(?:under|less than|below|max|maximum|up to)\s*\$?(\d{3,5})/);
    const maxPrice = priceMatch ? parseInt(priceMatch[1]) : null;
    // Extract bed filter if present (e.g. "3 bed", "3 bedroom", "3br")
